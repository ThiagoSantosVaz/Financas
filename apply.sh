...
original_author="ThiagoSantosVaz"
original_name="financas"
original_urlname="Financas"
original_description="Awesome financas created by ThiagoSantosVaz" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/financas "${name}"
...
then
    ...
    echo "then run 'financas' or 'python -m financas'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'financas' or 'python -m financas' or 'poetry run financas'"
fi
