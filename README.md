...
- 🎯 Entry points to execute your program using `python -m <financas>` or `$ financas` with basic CLI argument parsing.
...
---
# financas Flask Application

Awesome financas created by ThiagoSantosVaz
...
```bash
git clone https://github.com/ThiagoSantosVaz/Financas financas
cd financas
make install
```
...
```bash
financas create-db   # run once
financas populate-db  # run once (optional)
financas add-user -u admin -p 1234  # ads a user
financas run
```
